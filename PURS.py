...
def get_MACCS(ring_total_list,N):
    ...
    for idx,i in enumerate(ring_total_list):
        ...
        if not molecule:
            ...
            MACCS_dict[idx]=[0 for i in list(range(167))]
            continue
...
def get_feature(smi_list,name_list,matrix_list,pu_index,pair_atom_dist,total_inner_dist,MACCS_dict):
    dim_edge = 8
    #sdf_name = ''.join(chr(random.randint(ord('A'), ord('Z'))) for _ in range(6))+'.sdf'
    ...
    #structure = Chem.SDMolSupplier(sdf_name,removeHs=False)    
    node_feature_dist={}
    total_edge_list=[]
    edge_num_list = []
    senders_list={}
    ...
    for i, smi in enumerate(smi_list):
        ...
        smi = Chem.MolToSmiles(mol)
        index = []
        for k,v in pu_index[name].items():
            index.append(int(v))
        feature_list = []
        for j in index:
            f = MACCS_dict[j]
        ...
        feature = torch.tensor(feature_list)
        node_feature_dist[name] = feature
